...
public class Dklu_factor extends Dklu_internal
{
	 ...
	 * @param Common
	 */
	public static void factor2(int[] Ap, int[] Ai, Entry[] Ax,
		    KLU_symbolic Symbolic, KLU_numeric Numeric, KLU_common Common)
	{
	    ...
	    for (block = 0 ; block < nblocks ; block++)
	    {
	        ...
	        else
	        {
	            ...
	            Numeric.LUsize [block] = Dklu_kernel_factor.klu_kernel_factor (
	            		nk, Ap, Ai, Ax, Q,
	            ...
	            PRINTF ("\n----------------------- L %d:\n", block) ;
	            ASSERT (Dklu_valid_LU.klu_valid_LU (nk, TRUE, Lip+k1, Llen+k1,
	            		LUbx [block])) ;
	            PRINTF ("\n----------------------- U %d:\n", block) ;
	            ASSERT (Dklu_valid_LU.klu_valid_LU (nk, FALSE, Uip+k1, Ulen+k1,
	            		LUbx [block])) ;
	    ...
	    if (!NDEBUG)
	    {
	        PRINTF ("\n ############# KLU_BTF_FACTOR done, nblocks %d\n",
	        		nblocks);
	        Entry ss ;
	        Entry[] Udiag = Numeric.Udiag ;
	        for (block = 0 ; block < nblocks && Common.status == KLU_common.KLU_OK ; block++)
	        {
	            k1 = R [block] ;
	            k2 = R [block+1] ;
	            nk = k2 - k1 ;
		    ...
		    KLU_symbolic Symbolic, KLU_common Common)
	{
	    ...
	    Numeric.Offx = Dklu_memory.klu_malloc (nzoff1, sizeof (Entry), Common) ;
	    ...
	    Numeric.LUsize = Dklu_memory.klu_malloc (nblocks, sizeof (size_t), Common) ;

	    Numeric.LUbx = Dklu_memory.klu_malloc (nblocks, sizeof (Unit[]), Common) ;
	    ...
	    Numeric.Udiag = Dklu_memory.klu_malloc (n, sizeof (Entry), Common) ;
	     ...
	     */
	    s = Dklu_mult_size_t.klu_mult_size_t (n, sizeof (Entry), ok) ;
	    n3 = Dklu_mult_size_t.klu_mult_size_t (n, 3 * sizeof (Entry), ok) ;
	    ...
	    if (!ok || Common.status < KLU_common.KLU_OK)
	    {
	        /* out of memory or problem too large */
	        Common.status = ok == 1 ? KLU_common.KLU_OUT_OF_MEMORY :
	        		KLU_common.KLU_TOO_LARGE ;
	    ...
	    else if (Common.status == KLU_common.KLU_SINGULAR)
	    {
	        if (Common.halt_if_singular == 1)
	        {
	    ...
	    else if (Common.status == KLU_common.KLU_OK)
	    {
