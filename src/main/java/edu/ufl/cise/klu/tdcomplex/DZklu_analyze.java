...
import static edu.ufl.cise.klu.tdcomplex.DZklu_analyze_given.klu_z_analyze_given;
import static edu.ufl.cise.klu.tdcomplex.DZklu_analyze_given.klu_z_alloc_symbolic;
 ...
 */
public class DZklu_analyze extends DZklu_internal
{
			...
			int[] Pinv, KLU_symbolic Symbolic, KLU_common Common)
	{
		...
		for (block = 0 ; block < nblocks ; block++)
		{
			...
			Cp [nk] = pc ;
			maxnz = MAX (maxnz, pc) ;
			if (!NDEBUG) ASSERT (klu_z_valid (nk, Cp, Ci, null)) ;
	...
	public static KLU_symbolic order_and_analyze(int n, int[] Ap, int[] Ai, KLU_common Common)
	{
		...
		Symbolic = klu_z_alloc_symbolic (n, Ap, Ai, Common) ;
		if (Symbolic == null)
		{
			return (null) ;
		}
		...
		Pbtf = klu_z_malloc_int (n, Common) ;
		Qbtf = klu_z_malloc_int (n, Common) ;
		...
		Pblk = klu_z_malloc_int (maxblock, Common) ;
		Cp   = klu_z_malloc_int (maxblock + 1, Common) ;
		Ci   = klu_z_malloc_int (MAX (Cilen, nz+1), Common) ;
		Pinv = klu_z_malloc_int (n, Common) ;
	 ...
	 * @return null if error, or a valid KLU_symbolic object if successful
	 */
	public static KLU_symbolic klu_z_analyze(int n, int[] Ap, int[] Ai, KLU_common Common)
	{
		/* ---------------------------------------------------------------------- */
		...
		{
			/* natural ordering */
			return (klu_z_analyze_given (n, Ap, Ai, null, null, Common)) ;
		}
		else
		{
			/* order with P and Q */
