...
import static edu.ufl.cise.klu.tdcomplex.DZklu_memory.klu_z_malloc_int;
import static edu.ufl.cise.klu.tdcomplex.DZklu_memory.klu_z_malloc_dbl;
import static edu.ufl.cise.klu.tdcomplex.DZklu_memory.klu_z_add_size_t;
...
import static edu.ufl.cise.klu.tdcomplex.DZklu.klu_z_kernel_factor;

/**
 * Factor the matrix, after ordering and analyzing it with KLU_analyze
 * or KLU_analyze_given.
 */
public class DZklu_factor extends DZklu_internal
{
	 ...
	 * @param Common
	 */
	public static void factor2(final int[] Ap, final int[] Ai, final DZklua Ax,
			final KLU_symbolic Symbolic, KLU_numeric Numeric, KLU_common Common)
	{
		...
		if (scale >= 0)
		{
			...
			klu_z_scale (scale, n, Ap, Ai, Ax, Rs, Pnum, Common) ;
		...
		for (block = 0 ; block < nblocks ; block++)
		{
			...
			else
			{
				...
				Numeric.LUsize [block] = klu_z_kernel_factor (
						nk, Ap, Ai, Ax, Q,
				...
				PRINTF ("\n----------------------- L %d:\n", block) ;
				if (!NDEBUG) ASSERT (klu_z_valid_LU (nk, TRUE, Lip, k1, Llen, k1, LUbx [block])) ;
				PRINTF ("\n----------------------- U %d:\n", block) ;
				if (!NDEBUG) ASSERT (klu_z_valid_LU (nk, FALSE, Uip, k1, Ulen, k1, LUbx [block])) ;
	 ...
	 */
	public static KLU_numeric klu_z_factor(int[] Ap, int[] Ai, DZklua Ax,
			KLU_symbolic Symbolic, KLU_common Common)
	{
		...
		Numeric.nzoff = nzoff ;
		Numeric.Pnum = klu_z_malloc_int (n, Common) ;
		Numeric.Offp = klu_z_malloc_int (n1, Common) ;
		Numeric.Offi = klu_z_malloc_int (nzoff1, Common) ;
		Numeric.Offx = klu_z_malloc_dbl (nzoff1, Common) ;

		Numeric.Lip  = klu_z_malloc_int (n, Common) ;
		Numeric.Uip  = klu_z_malloc_int (n, Common) ;
		Numeric.Llen = klu_z_malloc_int (n, Common) ;
		Numeric.Ulen = klu_z_malloc_int (n, Common) ;

		Numeric.LUsize = klu_z_malloc_int (nblocks, Common) ;
		...
		Numeric.Udiag = klu_z_malloc_dbl (n, Common) ;
		...
		Numeric.Pinv = klu_z_malloc_int (n, Common) ;
		...
		b6 = 6 * maxblock ;
		Numeric.worksize = klu_z_add_size_t (s, MAX (n3, b6), ok) ;
		try
		{
