 ...
 *
 */
public class DZklu_sort extends DZklu_internal {
			...
			double[] LU, int[] Tp, int[] Tj, double[] Tx, int[] W)
	{
		...
		ASSERT (klu_z_valid_LU (n, FALSE, Xip, Xip_offset, Xlen, Xlen_offset, LU)) ;
	...
	public static int klu_z_sort(KLU_symbolic Symbolic, KLU_numeric Numeric,
			KLU_common Common)
	{
		...
		W  = klu_z_malloc_int (maxblock, Common) ;
		Tp = klu_z_malloc_int (m1, Common) ;
		Ti = klu_z_malloc_int (nz, Common) ;
		Tx = klu_z_malloc_dbl (nz, Common) ;
